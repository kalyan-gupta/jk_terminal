...
import threading
import time
from .kotak_websocket_lib import HSWebSocket, HSIWebSocket
...
class NeoWebSocket:
    ...
    def start_websocket_thread(self):
        if hasattr(self, 'hsw_thread') and self.hsw_thread and self.hsw_thread.is_alive():
            return
    ...
    def on_hsm_message(self, message):
        ...
        if message:
            if type(message) == str:
                ...
                if req_type == "unsub":
                    if len(self.un_sub_channel_token) > 0 and self.un_sub_channel and self.un_sub_channel in self.un_sub_channel_token:
    ...
    def start_hsi_websocket_thread(self):
        if hasattr(self, 'hsi_thread') and self.hsi_thread and self.hsi_thread.is_alive():
            return
